...
public:
  ...
  sampling_visualization(const Eigen::MatrixXd &location, const double &x_scale, const double &y_scale, const double &z_scale);

  void initialize_map(const std::string &visualization_frame, const std::string &name_space, const int &map_id, visualization_msgs::Marker &map);

  void update_map(const int &x_offset, const Eigen::VectorXd &filling_value, visualization_msgs::Marker &map);
